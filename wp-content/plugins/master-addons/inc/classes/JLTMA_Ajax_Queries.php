...
class JLTMA_Ajax_Queries
{
            ...
            if ($_POST['restrict_type'] == 'math_captcha') {
                if ($output['ma_el_rc_answer'] !== $output['ma_el_rc_answer_hd']) {
                    die(json_encode(array(
                        "result" => "validate",
                        "output" => sprintf(__('%1$s', JLTMA_TD), $_POST['error_message'])
                    )));
                }
            }
            ...
            if ($_POST['restrict_type'] == 'age_restrict') {
                ...
                if ($_POST['restrict_age']['age_type'] == "input_age") {
                    ...
                    if ($output['ma_el_ra_day'] == "" || $output['ma_el_ra_month'] == "" || $output['ma_el_ra_year'] == "") {
                        die(json_encode(array(
                            "result" => "validate",
                            "output" => sprintf(__('%1$s', JLTMA_TD), $_POST['restrict_age']['empty_bday'])
                        )));
                    } else if (!checkdate($output['ma_el_ra_month'], $output['ma_el_ra_day'], $output['ma_el_ra_year'])) {
                        die(json_encode(array(
                            "result" => "validate",
                            "output" => sprintf(__('%1$s', JLTMA_TD), $_POST['restrict_age']['non_exist_bday'])
                        )));
                    } else {
                        ...
                        if ($birthday > $min) {
                            die(json_encode(array(
                                "result" => "validate",
                                "output" => sprintf(__('%1$s , minimum age %2$s', JLTMA_TD), $min_age, $_POST['error_message'])
                            )));
                        }
                    }
                }
    ...
    public function jltma_domain_checker()
    {
        ...
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'jltma-domain-checker')) {
